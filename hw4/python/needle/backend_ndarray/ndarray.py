...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if prod(new_shape) != prod(self._shape):
            ...
            .format(prod(self._shape), prod(new_shape)))
        if not self.is_compact():
            raise ValueError("The matrix is not compact")
        new_strides = NDArray.compact_strides(new_shape)
        return NDArray.make(new_shape, new_strides, device=self._device, handle=self._handle, offset=self._offset)
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes):
        ...
        for i in range(len(self._shape)):
            if(i == new_axes[i]):
                ...
                from_idx = new_axes[i]
                strides[i] = self._strides[from_idx]
                shape[i] = self._shape[from_idx]
        shape = tuple(shape)
        strides = tuple(strides)
        return NDArray.make(shape, strides, device=self._device, handle=self._handle, offset=self._offset)
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        for i in range(len(shape)):
            if i >= len(strides):
                strides.append(0)
            elif shape[i] != 1 and shape[i] != new_shape[i]:
                raise ValueError("The {} idx was not equal {}:{}".format(i, self._shape[i], new_shape[i]))
            elif shape[i] == 1:
                strides[i] = 0
            else:
                pass
        strides = tuple(strides)
        return NDArray.make(new_shape, strides, device=self._device, handle=self._handle, offset=self._offset)
    ...
    def __getitem__(self, idxs):
        ...
        for i in range(len(idxs)):
            idx = idxs[i]
            length = len([x for x in range(idx.start, idx.stop, idx.step)])
            
            offset += self._strides[i] * idx.start
            shape.append(length)
            strides.append(self._strides[i] * idx.step)
        shape = tuple(shape)
        strides = tuple(strides)

        return NDArray.make(shape, strides, device=self._device, handle=self._handle, offset=offset)
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        """
        new_shape = [x for x in self._shape]
        for i in range(len(new_shape)):
            new_shape[i] += axes[i][0] + axes[i][1]
        array = self.make(tuple(new_shape), device=self._device)
        init_slices = []
        for i in range(len(new_shape)):
        ...
        set_slices = []
        for i in range(len(new_shape)):
            set_slices.append(slice(axes[i][0], new_shape[i] - axes[i][1], 1))
        array.__setitem__(tuple(set_slices), self)
        return array
