...
class Value:
    ...
    def astype(self, dtype):
        self.cached_data = self.cached_data.astype(dtype)
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    ### BEGIN YOUR SOLUTION
    for value in reverse_topo_order:
      if(not value.requires_grad):
        continue
      grads = node_to_output_grads_list[value]
      grad = grads[0]
      for j in range(1, len(grads)):
        grad = grad + grads[j]
      # Store the grad
      value.grad = grad
      # Skip when the operation is None
      if(value.op == None):
        continue
      adjoint_grads = value.op.gradient_as_tuple(grad, value)

      for i in range(len(value.inputs)):
        k_value = value.inputs[i]
        k_grad = adjoint_grads[i]
        if k_value not in node_to_output_grads_list:
          node_to_output_grads_list[k_value] = [k_grad]
        else:
          node_to_output_grads_list[k_value].append(k_grad)
    ### END YOUR SOLUTION
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    for value in node_list:
      visited = {}
      topo_order = []
      topo_sort_dfs(value, visited, topo_order)
