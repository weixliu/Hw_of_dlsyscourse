...
std::vector<uint32_t> get_compact_strides(std::vector<uint32_t> shape) {
  std::vector<uint32_t> compact_strides;
  uint32_t stride = 1;
  for(int i = 1;i <= shape.size();i++) {
    compact_strides.push_back(stride);
    stride *= shape[shape.size() - i];
  }
  reverse(compact_strides.begin(), compact_strides.end());
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape, CudaVec compact_strides,
...
void Compact(const CudaArray& a, CudaArray* out, std::vector<uint32_t> shape,
  ...
  std::vector<uint32_t> compact_strides = get_compact_strides(shape);

  CompactKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, out->size, VecToCuda(shape), VecToCuda(compact_strides),
                                         VecToCuda(strides), offset);
}

__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape, CudaVec compact_strides,
                              CudaVec strides, size_t offset) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
void EwiseSetitem(const CudaArray& a, CudaArray* out, std::vector<uint32_t> shape,
  ...
  EwiseSetitemKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, a.size, VecToCuda(shape), VecToCuda(compact_strides),
                                         VecToCuda(strides), offset);
  /// END YOUR SOLUTION
}

__global__ void ScalarSetitemKernel(const scalar_t val, scalar_t* out, size_t size, CudaVec shape, CudaVec compact_strides,
                              CudaVec strides, size_t offset) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
void ScalarSetitem(size_t size, scalar_t val, CudaArray* out, std::vector<uint32_t> shape,
  ...
  ScalarSetitemKernel<<<dim.grid, dim.block>>>(val, out->ptr, size, VecToCuda(shape), VecToCuda(compact_strides),
                                         VecToCuda(strides), offset);
  /// END YOUR SOLUTION
}
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, uint32_t M, uint32_t N, uint32_t P, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    scalar_t ele = 0.0;
...
__global__ void ReduceMaxKernel(const scalar_t* a, size_t reduce_size, scalar_t* out, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    scalar_t ele = a[gid * reduce_size];
    for(int i = gid * reduce_size;i < gid * reduce_size + reduce_size;i++) {
      ele = fmax(ele, a[i]);
    }
    out[gid] = ele;
  }
...
void ReduceMax(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  /// BEGIN YOUR SOLUTION
  CudaDims dim = CudaOneDim(out->size);
  ReduceMaxKernel<<<dim.grid, dim.block>>>(a.ptr, reduce_size, out->ptr, out->size);
  /// END YOUR SOLUTION
}


__global__ void ReduceSumKernel(const scalar_t* a, size_t reduce_size, scalar_t* out, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    scalar_t ele = 0.0;
    for(int i = gid * reduce_size;i < gid * reduce_size + reduce_size;i++) {
...
void ReduceSum(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  /// BEGIN YOUR SOLUTION
  CudaDims dim = CudaOneDim(out->size);
  ReduceSumKernel<<<dim.grid, dim.block>>>(a.ptr, reduce_size, out->ptr, out->size);
  /// END YOUR SOLUTION
}
