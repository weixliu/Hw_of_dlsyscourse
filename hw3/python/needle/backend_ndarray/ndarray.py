...
class NDArray:
    ...
    def broadcast_to(self, new_shape):
        ...
        strides = [x for x in self._strides]
        for i in range(len(self._shape)):
            if self._shape[i] != 1 and self._shape[i] != new_shape[i]:
    ...
    def __matmul__(self, other):
        ...
        if hasattr(self.device, "matmul_tiled") and all(
            ...
            # print((self.compact() == 1).sum(), m * n)
            # print((other.compact() == 1).sum(), n * p)
            # t = self.device.__tile_size__
            # print((tile(self.compact(), t) == 1).sum(), m * n)
            # print((tile(other.compact(), t) == 1).sum(), n * p)
            t = self.device.__tile_size__
